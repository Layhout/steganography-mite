...
class ImagePreview(customtkinter.CTkFrame):
    ...
    def set_image(self):
        ...
        is_valid_file_type = any(self.main_image_file_path.endswith(t) for t in list(VALID_IMAGE_FILE_TYPE.values()))
        if not is_valid_file_type:
            return
        file = open(self.main_image_file_path, 'rb')
        image = Image.open(file, 'r')
        if image == 'P':
            messagebox.showerror('Invalid Image', 'Color channel of the image is not supported.')
            return
        ...
        self.image_preview_footer = ImagePreviewFooter(self, self.main_image_filename, self.set_image)
...
class ImagePlaceholder(customtkinter.CTkFrame):
    def __init__(self, parent, import_image_command):
        ...
        self.import_label = customtkinter.CTkLabel(self, text='Select an main image.', font=customtkinter.CTkFont(size=16, weight="bold"))
...
class ImagePreviewFooter(customtkinter.CTkFrame):
    def __init__(self, parent, filename, import_image_command):
        ...
        self.imported_label = customtkinter.CTkLabel(self, text=self.filename, anchor='w')
        self.imported_label.grid(row=0, column=0, sticky="we")
        self.import_button = customtkinter.CTkButton(self, text="Change Image", command=self.import_image_command)
