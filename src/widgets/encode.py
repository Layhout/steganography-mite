...
class Encode(customtkinter.CTkFrame):
    def __init__(self, parent):
        ...
        self.action_frame = customtkinter.CTkFrame(self, width=300, corner_radius=0, border_width=1, border_color=('grey75', 'grey25'))
        ...
        self.method_label = customtkinter.CTkLabel(self.action_frame, text="Method", anchor='w', font=customtkinter.CTkFont(size=14, weight="bold"))
        self.method_label.grid(row=0, column=0, padx=20, pady=10, sticky="we")

        self.method_optionmenu = customtkinter.CTkOptionMenu(self.action_frame, values=STEGANOGRAPHY_METHODS, command=self.change_method_event)
        self.method_optionmenu.grid(row=1, column=0, padx=20, pady=(0, 10), sticky="we")
        self.new_method = STEGANOGRAPHY_METHODS[0]

        self.data_type_label = customtkinter.CTkLabel(self.action_frame, text="Data Type", anchor='w', font=customtkinter.CTkFont(size=14, weight="bold"))
        self.data_type_label.grid(row=2, column=0, padx=20, pady=10, sticky="we")

        self.data_type_optionmenu = customtkinter.CTkOptionMenu(self.action_frame, values=["Text", "File"], command=self.change_data_type_event)
        self.data_type_optionmenu.grid(row=3, column=0, padx=20, pady=(0, 10), sticky="we")
        self.data_type = 'Text'

        self.data_label = customtkinter.CTkLabel(self.action_frame, text="Data to Encode", anchor='w', font=customtkinter.CTkFont(size=14, weight="bold"))
        self.data_label.grid(row=4, column=0, padx=20, pady=10, sticky="we")

        self.text_entry = customtkinter.CTkTextbox(self.action_frame, border_color=('grey75', 'grey25'), border_width=1)
        self.text_entry.grid(row=5, column=0, padx=20, pady=(0, 20), sticky="we")

        self.filename = ''
        self.file = None
        self.import_file_button = customtkinter.CTkButton(self.action_frame, text="Import File", command=self.import_file_event)
        self.filename_label = customtkinter.CTkLabel(self.action_frame, text=self.filename, anchor='e')

        self.encode_button = customtkinter.CTkButton(self.action_frame, text="Encode", command=self.encode_event)
    ...
    def change_data_type_event(self, new_type):
        ...
        if new_type == 'Text':
            ...
            self.import_file_button.grid(row=5, column=0, padx=20, pady=(0, 10), sticky="we")
            self.filename_label.grid(row=6, column=0, padx=20, pady=(0, 20), sticky="we")

    def change_method_event(self, new_method):
        self.ste_method = new_method

    def import_file_event(self):
        path = filedialog.askopenfile(filetypes=[(k, v) for k, v in VALID_ENCODE_FILE_TYPE.items()])
        if path is None:
            return
        self.filename = path.name
        self.filename_label.configure(text=self.filename)
        is_valid_file_type = any(self.filename.endswith(t) for t in list(VALID_ENCODE_FILE_TYPE.values()))
    ...
    def encode_event(self):
        ...
        match self.data_type:
            case 'Text':
                if  self.text_entry.get('1.0', 'end-1c') == '':
                    ...
                    return
                else: secret_data = self.text_entry.get('1.0', 'end-1c')
        ...
        secret_token = customtkinter.CTkInputDialog(text="Input a secret word", title="Secret Word").get_input()
        if secret_token == '': 
        ...
        match self.method_optionmenu.get():
            case 'LSB':
                ste_status = ste.lsb_encode(secret_data, secret_token, self.image_preview.main_image_filename)
                
        if ste_status == 'fail': return
