...
from src.constants import VALID_ENCODE_FILE_TYPE
...
class Steganography:
    ...
    def __ask_for_file_path(self, filename: str) -> str:
        ...
        filename_without_extension = '.'.join(split_filename[:-1])
        current_timestamp = int(time.time())
        path = filedialog.asksaveasfilename(initialfile=f'{filename_without_extension}-ste-de-{current_timestamp}', filetypes=[('PNG', '.png')])
        return path
    ...
    def lsb_encode(self, message: str, stop_token: str, encoded_file_name: str) -> str:
        ...
        for i in range(self.image_pixel):
            for j in range(0, 3):
                if index < bit:
                    self.image_arr[i][j] = int(bin(self.image_arr[i][j])[2:-2] + byte_message[index:index + 2], 2)
                    index += 2
        
        self.image_arr = self.image_arr.reshape(self.image_height, self.image_width, self.image_channel)
        ...
        if save_path == '': return 'fail'

        result.save(save_path, quality='keep')
        return 'success'

    def lsb_decode(self, stop_token: str):
        secret_bit = [bin(self.image_arr[i][j])[2:].zfill(8)[-2:] for i in range(self.image_pixel) for j in range(0, 3)]
